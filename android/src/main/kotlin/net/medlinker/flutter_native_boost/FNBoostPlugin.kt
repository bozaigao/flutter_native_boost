...
/** FNBoostPlugin */
class FNBoostPlugin : FlutterPlugin, MethodCallHandler, ActivityAware {

    companion object {
        private const val TAG = "FNBoostPlugin"
    }

    private lateinit var channel: MethodChannel

    private var navigator: ContainerNavigator? = null
        ...
        when (call.method) {
            "pushNativePage" -> {
                ...
                navigator?.create(name, arguments, Options(options))?.let {
                    Container.startNativeForResult(it) { nativeResult ->
            ...
            "reCreateLastPage" -> {
                when (val activity = Container.getCurrentActivity()) {
                    is ContainerActivity -> {
    ...
    override fun onAttachedToEngine(binding: FlutterPlugin.FlutterPluginBinding) {
        channel = MethodChannel(binding.binaryMessenger, "flutter_native_boost")
        channel.resizeChannelBuffer(2)
        channel.setMethodCallHandler(this)
        this.navigator = Container.navigator
        Container.pluginRef = WeakReference(this)
    }
