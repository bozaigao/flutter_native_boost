...
`flutter_native_boost`默认现有的`ios`&`android`已经存在大量的网络基础设施(包括但不限于登录验证，加解密等等)

> 如果没有没有你可以跳过此部分内容

## Flutter

`flutter`侧我们提供了`ContainerNet`用来进行网络请求

``` dart

final response = await ContainerNet.request('get', '/user', parameters: {'id': 1}, headers: {'ua': 'flutter_native_boost'});
...
extension AppDelegate: ContainerHttpProvider {
...
// 使用
Container.default.startFlutterEngine(navigatorDelegate: self, httpProvider: self, commonHandler: self.handle(_:_:_:), automaticallyRegisterPlugins: true)
...
//
class ContainerNet : NetHandler {
    private val api = Net.create<ContainerFutureApi>()

    interface ContainerFutureApi {
...
//
Container.setNetHandler(ContainerNet())
