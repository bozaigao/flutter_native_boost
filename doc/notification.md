...
// 发送 (只会发送到flutter，如需发送到native请自行广播)
Container.postNotification("NotificationKey", mapOf("id" to 1234))

// 接收 (只能接收flutter侧通知)
Container.registerNotification("NotificationKey") { params ->
    //解除注册
    Container.unregisterNotification("NotificationKey")
}
