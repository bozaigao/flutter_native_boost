...
class _SplashTipPageState extends State<SplashTipPage> {
  ...
  List<Widget> _buildAndroidTip(BuildContext context) {
    return [
      ...
      Text('3. 继承ContainerActivity然后实现 provideSplashScreen'),
      TextButton(child: Text('查看 ContainerActivity.kt'), onPressed: () {}),
    ];
  }
  ...
  List<Widget> _buildFlutterTip(BuildContext context) {
    return [
      ...
      TextButton(
          child: Text('查看`main.dart`->`Container.wrapper'), onPressed: () {})
    ];
  }
}
