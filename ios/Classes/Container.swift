...
public protocol ContainerNavigationDelegate: AnyObject {
    ...
    /// flutter widget request pop(dismiss) current ContainerFlutterViewcontroller
    /// - Parameters:
    ///   - viewController: current viewcontroller attached on engine
    /// - Returns: pop succeed or not
    func pop(_ viewController: ContainerFlutterViewController) -> Bool
}

public extension ContainerNavigationDelegate {
    
    func disableHorizontalSwipePopGesture(_ disable: Bool) {
        Container.default.currentFlutterViewController?.disableHorizontalSwipePopGesture(disable: disable)
    }
...
public protocol ContainerHttpProvider: AnyObject {
...
public typealias ContainerHandler = (_ name: String, _ arguments: Any?, _ completion: @escaping (_ result: Any?) -> Void) -> Void
...
public class Container {
    ...
    private init() {
        debugPrint("🚀🚀Container default instance initailed !")
    }
    
    private weak var navigatorDelegate: ContainerNavigationDelegate? // not retain
    private weak var netProvider: ContainerHttpProvider?
    
    private var commonHandler: ContainerHandler?
    ...
    public var currentFlutterViewController: ContainerFlutterViewController? {
        return engine?.viewController as? ContainerFlutterViewController
    }
    
    //
    func setup(messenger: FlutterBinaryMessenger) {
        
        channel = FlutterMethodChannel(name: "flutter_native_boost", binaryMessenger: messenger)
        ...
        notificationChannel = FlutterMethodChannel(name: "flutter_native_boost/notification", binaryMessenger: messenger)
        ...
        netChannel = FlutterMethodChannel(name: "flutter_native_boost/net", binaryMessenger: messenger)
        ...
        if let h = commonHandler {
            commonChannel = FlutterMethodChannel(name: "flutter_native_boost/common", binaryMessenger: messenger)
    ...
    ///
    public func startFlutterEngine(navigatorDelegate: ContainerNavigationDelegate,
                                   httpProvider: ContainerHttpProvider? = nil,
                                   commonHandler: ContainerHandler? = nil,
        ...
        engine = FlutterEngine(name: "io.flutter.container", project: nil, allowHeadlessExecution: true)
    ...
    /// 强制刷新某个 `ContainerFlutterViewController` 实例的渲染状态。 一般情况下不需要调用此方法。
    /// 推荐在ContainerFlutterViewController生命周期没有按照预期执行的时候调用
    /// 例如 在iOS13 以上从ContainerFlutterViewController `present`新页面的时候 生命周期调用不符合预期
    /// - Parameter viewController: 需要刷新状态的ContainerFlutterViewController实例
    public static func refreshViewController(_ viewController: ContainerFlutterViewController) {
    ...
    static func callback(_ token: CallbackToken?, result: Any?) {
        if let t = token {
            if let cb = Container.default.callbackCache.removeValue(forKey: t) {
                cb(result)
            }
        }
    }
    ...
    func pop(flutterContainer arguments: Any?, callback: FlutterResult) {
        guard let viewController = currentFlutterViewController else {
            debugPrint("[Container] Error current flutter viewController not found. can't pop");
            callback(false)
            return
        }
    ...
    func disableHorizontalSwipePopGesture(arguments: Any?, callback: FlutterResult) {
        Container.default.navigatorDelegate?.disableHorizontalSwipePopGesture(arguments as? Bool ?? false)
        callback(true)
    }
...
//                return ("pageHidden", seq)
    ...
    static func sendPageState(_ state: Container.PageState, result: @escaping (Bool) -> Void) {
        let container = Container.default
        let info = state.info;
        container.channel?.invokeMethod(info.0, arguments: info.1, result: { r in
