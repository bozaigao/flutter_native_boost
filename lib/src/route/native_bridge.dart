...
const _channel = MethodChannel('flutter_native_boost');
...
class ContainerNativeBridge extends StatefulWidget {
  ...
  static ContainerNativeBridgeState? of(BuildContext context) {
    if (context is StatefulElement &&
        context.state is ContainerNativeBridgeState) {
      return context.state as ContainerNativeBridgeState;
    }
    return context.findAncestorStateOfType<ContainerNativeBridgeState>();
  }

  @override
  ContainerNativeBridgeState createState() => ContainerNativeBridgeState();
}

class ContainerNativeBridgeState extends State<ContainerNativeBridge> {
  final List<ContainerArguments> _navigators = [];
  int? _index;
  ...
  GlobalKey<ContainerNavigatorState>? get topNavigator =>
        ...
        return Container(
          ...
          child: Center(
            child: Wrap(
              ...
              children: [
                Text(
                  'g_Container 路由栈错误，请确认非 hot-restart 引起',
                  style: style,
                ),
                Text(
  ...
  Future<bool> _handler(MethodCall call) async {
    debugPrint('channel: g_Container method: ${call.method}: ${call.arguments}');
    switch (call.method) {
      case 'pageCreate':
        ...
        final arg = ContainerArguments(call.arguments['args'], name, id,
  ...
  Widget _buildPage(BuildContext context, ContainerArguments arg) {
